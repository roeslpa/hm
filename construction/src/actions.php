...
if($a!='')
{
	if($a == 'login') {
		...
		$sql4 = " ((  '".$_POST['gamePassword']."' in (select `gpwd` from (select `gpwd` from `hm` where `id` = '".$changing_g_id."') as `temptable`) ) and";	//password correct?		#Issue: pass hash
		...
		$sql5 = " ('".$_POST['gamePassword']."' = '' and '".$_GET['leave']."' = '1')) and ";		//leave game
		...
		$query = mysql_query("update `hm` set `gid` = `id`, `leader` = '1', `gpwd` = '".$_POST['gamePassword'].
		"', `gend` = '1', `letters` = '', `word` = '".strtoupper($_POST['word'])."' where `id`='".$userId."'");		// #Issue: GPWD hashen
